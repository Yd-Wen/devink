...
"""
注意 alias 参数的用法: 
- Python 约定使用下划线命名(user_account)
- 但前端和数据库使用驼峰命名(userAccount)
- 通过 alias 设置后，接口接收和返回的 JSON 字段使用驼峰，Python 代码内部使用下划线，两边都保持了各自的命名习惯。
"""
...
"""
uservo 和 Loginuservo 是脱敏后返回给前端的视图对象，不包含密码等敏感字段。 
Loginuservo 比 uservo 多了update_time 字段，用于展示登录用户自己的信息。
"""
