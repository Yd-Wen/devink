...
from app.agent.orchestrator import BlogAgentOrchestrator
...
from app.models.enums import SseMessageTypeEnum, ImageMethodEnum, BlogStyleEnum
...
class BlogAgentService:
    ...
    def __init__(self):
        ...
        # 初始化策略模式
        self.image_strategy = ImageStrategy()
        self.agent_log = AgentLogService(database)
        ...
        self.orchestrator = BlogAgentOrchestrator()
    ...
    async def agent4_analyze_image_requirements(self, state: BlogState):
        ...
        prompt = (
            PromptConstant.IMG_REQ_AGENT_PROMPT
            .replace("{mainTitle}", state.title.main_title)
    ...
    def _get_style_prompt(self, style: Optional[str]) -> str:
        ...
        try:
            style_enum = BlogStyleEnum(style)
            style_map = {
                BlogStyleEnum.TECH: PromptConstant.STYLE_TECH_PROMPT,
                BlogStyleEnum.EMOTIONAL: PromptConstant.STYLE_EMOTIONAL_PROMPT,
                BlogStyleEnum.EDUCATIONAL: PromptConstant.STYLE_EDUCATIONAL_PROMPT,
                BlogStyleEnum.HUMOROUS: PromptConstant.STYLE_HUMOROUS_PROMPT,
            }
   ...
   - SVG_DIAGRAM: 适合概念示意图、思维导图样式、逻辑关系展示（不涉及精确数据）"""
    ...
    async def _agent_log_context(
        ...
        try:
            ...
            self.agent_log.save_log_async(log_data)
