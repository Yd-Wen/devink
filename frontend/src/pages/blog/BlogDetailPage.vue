  ...
  <div class="blog-detail-page">
    ...
    <div class="container">
      <a-spin :spinning="loading" tip="加载中...">
        <a-card :bordered="false" v-if="blog" class="blog-card">
          <!-- 标题 -->
          <div class="title-section">
            <h1 class="main-title">{{ blog.mainTitle }}</h1>
            <p class="sub-title">{{ blog.subTitle }}</p>
            <div class="meta-info">
              <a-tag :color="getStatusColor(blog.status ?? '')" class="status-tag">
                {{ getStatusText(blog.status ?? '') }}
              </a-tag>
              <span class="time">创建于 {{ blog.createTime ? formatDate(blog.createTime) : '' }}</span>
            </div>
          </div>
          ...
          <a-divider v-if="blog.outline && blog.outline.length > 0" />

          <!-- 完整图文（优先展示） -->
          <div v-if="blog.fullContent" class="content-section">
            <h2 class="section-title">
              <FileTextOutlined class="section-icon" />
              完整图文
            </h2>
            <div v-html="markdownToHtml(blog.fullContent)" class="markdown-content"></div>
          </div>
...
import { getBlog, getExecutionLogs } from '@/api/blogController'
...
const blog = ref<API.BlogVO | null>(null)
...
const loadBlog = async () => {
  ...
  loading.value = true
  try {
    const res = await getBlog({ taskId })
    blog.value = res.data.data || null
    // 自动加载执行日志
...
const exportMarkdown = () => {
  ...
  let markdown = `# ${blog.value.mainTitle}\n\n`
  markdown += `> ${blog.value.subTitle}\n\n`

  // 优先使用完整图文
  if (blog.value.fullContent) {
    markdown += blog.value.fullContent
  } else {
    if (blog.value.outline && blog.value.outline.length > 0) {
      markdown += `## 目录\n\n`
      blog.value.outline.forEach(item => {
  ...
  a.href = url
  a.download = `${blog.value.mainTitle}.md`
  a.click()
